...
export const getAll = async (req, res) => {
    try {
        const sortOption = req.query.sort === "popular" ? { viewsCount: -1 } : { createdAt: -1 };
...
export const getOne = async (req, res) => {
    try {
        ...
        if (userId && post.user._id.toString() !== userId) {
            const hasViewed = post.views.some((id) => id.toString() === userId);
            if (!hasViewed) {
...
export const like = async (req, res) => {
    try {
        const postId = req.params.id;
        const userId = req.userId;
        const post = await PostModel.findOneAndUpdate({ _id: postId, likes: { $ne: userId } }, { $addToSet: { likes: userId } }, { new: true });
        if (!post) {
            res.status(409).json({
...
export const unlike = async (req, res) => {
    try {
        const postId = req.params.id;
        const userId = req.userId;
        const post = await PostModel.findOneAndUpdate({ _id: postId, likes: userId }, { $pull: { likes: userId } }, { new: true });
        if (!post) {
